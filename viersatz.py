...
notes = np.array([note+str(i) for i in range(total_octaves) for note in base_notes])
leiter = np.array(total_octaves*base_leiter)
accord_helper = np.array(2*total_octaves*(1,0,0,1,1,0,0,1,0,0,0,0))
sept_accord_helper = np.array(2*total_octaves*(1,0,0,1,1,0,0,1,0,0,0,1))
...
def next(prev, grundton, start=None, temperature=100):
    ...
    for i in [4,1,3,2]:
        ...
        if start is not None:
            ...
            for k in range(size):
                temp_modifier[k] = np.exp(-np.abs(k-start[i])/6*100/temperature)
            #print(temp_modifier)
            #input()
            p = p*temp_modifier
...
"""    <score-part id="P1">
    ...
    content = content[:plist_pos+len(seekstring)] + extra_parts + content[plist_pos+len(seekstring):]
